...
select distinct 
user_id 
from credit_risk_playground.bp_portfolio_customers_aud_m_ik
...
--          and ps.score_type in ('behavioral_unarranged', 'behavioral')
          and ps.created::date = '2025-06-11'::date 
)
...
from porto p 
    ...
    where reporting_date > (select max(reporting_date) from "n26"."credit_risk_playground"."bp_monitoring_pd_supermodel_v2_m_ik")
