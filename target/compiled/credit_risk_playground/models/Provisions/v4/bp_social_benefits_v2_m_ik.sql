...
WHERE bank_balance_impact > 0 
      ...
      AND t.created::date between date_add('day', - 30, '2025-06-11'::date
                                        )::date and '2025-06-11'::date 
group by 1
)
...
where etl_updated > (select max(etl_updated) from "n26"."credit_risk_playground"."bp_social_benefits_v2_m_ik")
