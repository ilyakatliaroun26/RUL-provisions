...
bp_portfolio_balance_aud_m_ik +
bp_overdraft_limit_periods_ik   (first drop then create again) +
bp_portfolio_customers_aud_m_ik + 
bp_overdue_amt_aud_m_ik  (is not used anymore) skipped
bp_arrears_reg_aud_ik  (use dbt.bp_arrears_reg_aud instead) skipped
bp_overdraft_split_m_ik +
bp_monitoring_pd_supermodel_v2_m_ik +
bp_credit_bureau_rating_m_ik  +
bp_default_dunning_m_ik +
bp_default_internal_insolvency_v2_m_ik +
bp_default_credit_bureau_insolvency_max_date_ik (first drop) +
bp_default_credit_bureau_insolvency_rmd_ik (first drop) +
bp_default_credit_bureau_insolvency_me_ik (first drop) +
bp_default_credit_bureau_insolvency_v3_m_ik +
bp_repayment_plan_ph1_m_ik +
bp_loan_contract_information_m_ik +
bp_p_accounts_ik (first drop) +
bp_hrc_m_ik +
bp_social_benefits_v2_m_ik +
bp_garnishment_m_ik +
bp_application_pd_supermodel_m_ik +
bp_application_pd_supermodel_m_ad_ik +
bp_retail_provisions_stg_v5_pre_updated_ik +
bp_forbearance_m_v2_ik +
bp_retail_provisions_stg_v5_v4_updated_ik +
bp_retail_provisions_m_ik +
*/
...
, case when product_flag in ('Arranged Overdraft', 'RP_1', 'RP_2') and sum(case when product_flag in ('Arranged Overdraft', 'RP_1', 'RP_2') then coalesce(OnBalance_EAD,0) else 0 end) over(partition by user_id) < 500.22 then 0.461767::float
...
, GLLP_scen_weighted as GLLP
-- ECL split 


, ECL_On_scen_weighted as ECL_On

, ECL_Off_scen_weighted as ECL_Off


from "n26"."credit_risk_playground"."bp_retail_provisions_stg_v5_v4_updated_ik" 
where ReportDate::date = '2025-06-11'::date
and case when onbalance_ead = 0 and "limit" = 0 then 1 else 0 end <> 1
)
...
where etl_updated > (select max(etl_updated) from "n26"."credit_risk_playground"."bp_retail_provisions_m_ik")
