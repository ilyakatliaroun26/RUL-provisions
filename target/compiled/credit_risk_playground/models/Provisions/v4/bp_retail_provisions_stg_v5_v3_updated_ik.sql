...
, bp_retail_provisions_stg_v5_pre_updated_complete as (
select user_id, instrument_id, country, is_ifrs, reportdate, calcdate, Script_calculation_date, product_flag, Instrument_Type, Instrument
, credit_creation_date, start_in_arrears, dpd, insolvency, dunning, schufa_insolvency, rep_plan_user, is_default, schufa_rating, "provider"
, schufa_rating_date, "limit", last_limit_date, TBIL_exposures, CC_exposures, RP_2_exposures, OD_exposures, porto_calibration_segment, internal_pd
, logit, pd_pit_calibrated, pd_pit_calibrated_down, pd_pit_calibrated_neu, pd_pit_calibrated_up, pd_pit_calibrated_scen_weighted, rating_class_pit_calibrated
, rating_class_pit_calibrated_down, rating_class_pit_calibrated_neu, rating_class_pit_calibrated_up, rating_class_pit_calibrated_scen_weighted, pd, pd_down,
pd_neu, pd_up, PD_scen_weighted, etl_updated, unique_key, LegalEntity, pit_calibration_segment
from credit_risk_playground.bp_retail_provisions_stg_v5_pre_updated
...
from "n26"."credit_risk_playground"."bp_retail_provisions_stg_v5_pre_updated_ik"
)
...
from bp_retail_provisions_stg_v5_pre_updated_complete c 
...
, case when (df.instrument_id is not null)
...
, case when d.user_id is not null or ins.user_id is not null or sch.user_id is not null then '19'
       when (df.instrument_id is not null)
...
left join "n26"."credit_risk_playground"."bp_p_accounts_ik" pa on pa.user_id = c.user_id and last_day(date_add('month', -1, last_day(getdate()))) between pa.rev_timestamp and pa.end_timestamp
...
left join latest_default_during_forbearance df on df.instrument_id = c.instrument_id
left join default_last_12_months ld on ld.instrument_id = c.instrument_id
...
where etl_updated > (select max(etl_updated) from "n26"."credit_risk_playground"."bp_retail_provisions_stg_v5_v3_updated_ik")
