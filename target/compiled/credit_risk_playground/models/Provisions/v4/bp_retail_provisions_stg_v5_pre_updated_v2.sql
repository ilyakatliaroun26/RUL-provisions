...
left join "n26"."credit_risk_playground"."bp_portfolio_balance_aud_m_v2" o on o.encoded_key = c.encoded_key and c.reporting_date::date = o.reporting_date::date
left join "n26"."credit_risk_playground"."bp_application_pd_supermodel_m_v2" pd_app on pd_app.encoded_key = c.encoded_key 
                                                                and pd_app.reporting_date::date = c.reporting_date::date
left join "n26"."credit_risk_playground"."bp_overdraft_split_m_v2"  mll on mll.encoded_key = c.encoded_key and c.reporting_date::date = mll.reporting_date::date
left join "n26"."credit_risk_playground"."bp_monitoring_pd_supermodel_v2_m_v2" m on m.user_id = c.user_id and m.reporting_date::date = c.reporting_date::date
-- to be fixed - application scores should be from porto
left join "n26"."credit_risk_playground"."bp_default_dunning_m_v2" d on d.user_id = c.user_id  and d.reporting_date::date = c.reporting_date::date   
left join "n26"."credit_risk_playground"."bp_default_internal_insolvency_v2_m_v2" ins on ins.user_id = c.user_id and ins.reporting_date::date = c.reporting_date::date
left join "n26"."credit_risk_playground"."bp_default_credit_bureau_insolvency_v3_m_v2" sch on sch.user_id = c.user_id and sch.reporting_date::date = c.reporting_date::date
left join "n26"."credit_risk_playground"."bp_repayment_plan_ph1_m_v2" rp on rp.user_id = c.user_id and rp.reporting_date::date = c.reporting_date::date
left join "n26"."credit_risk_playground"."bp_credit_bureau_rating_m_v2" schr on schr.user_id = c.user_id and schr.reporting_date::date = c.reporting_date::date
...
left join "n26"."credit_risk_playground"."bp_overdraft_limit_periods_v2" p on c.encoded_key = p.encoded_key 
...
left join "n26"."credit_risk_playground"."bp_loan_contract_information_m_v2" li on li.encoded_key = c.encoded_key and li.reporting_date::date = c.reporting_date::date
...
where etl_updated > (select max(etl_updated) from "n26"."credit_risk_playground"."bp_retail_provisions_stg_v5_pre_updated_v2")
