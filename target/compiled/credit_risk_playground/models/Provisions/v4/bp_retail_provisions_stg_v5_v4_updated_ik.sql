...
where rev_timestamp <= date_add('minute', -1, date_add('day', 1, '2025-06-11'::date) ) and amount_cents > 0 
group by 
user_id 
, encoded_key
)
...
select distinct 
instrument_id
from "n26"."credit_risk_playground"."bp_forbearance_m_v2_ik" f 
where f.credit_creation_date::date between date_add('month', -12
                                                    , '2025-06-11'::date
                                                    ) and '2025-06-11'::date
      and f.reportdate = '2025-06-11'::date
...
from "n26"."credit_risk_playground"."bp_retail_provisions_stg_v5_pre_updated_ik" c 
left join "n26"."credit_risk_playground"."bp_forbearance_m_v2_ik" f on c.instrument_id = f.instrument_id and c.reportdate::date = f.reportdate::date
...
, case when d.user_id is not null or ins.user_id is not null or sch.user_id is not null then '2025-06-11'::date
       when (df.user_id is not null)
            and (c.reportdate::date between df.default_during_forbearance_date::date and date_add('month', 12, df.default_during_forbearance_date::date)) then '2025-06-11'::date
       when c.dpd > 90 then '2025-06-11'::date
       when max(case when c.product_flag in ('RP_2') then 1 else 0 end) over(partition by c.user_id) = 1 then
              case when c.dpd > 90 then '2025-06-11'::date
                  when c.dpd > 60 then  '2025-06-11'::date
                  when c.dpd > 30 then '2025-06-11'::date
                  when c.dpd > 0 then '2025-06-11'::date
                  --when c.rating_class_pit_calibrated < 14 then m.calculated_at::date 
                  else '2025-06-11'::date
               end 
       ...
       when c.pd_pit_calibrated is null then 
            case when c.dpd > 90 then '2025-06-11'::date
                 ...
                 when c.dpd > 0 then '2025-06-11'::date
                     ...
                     case when schr.rating = 'A' then '2025-06-11'::date
                          when schr.rating = 'B' then '2025-06-11'::date
                          when schr.rating = 'C' then '2025-06-11'::date
                          when schr.rating = 'D' then '2025-06-11'::date
                          when schr.rating = 'E' then '2025-06-11'::date
                          when schr.rating = 'F' then '2025-06-11'::date
                          when schr.rating = 'G' then '2025-06-11'::date
                          when schr.rating in ('H','I','K','L', 'M','UNKNOWN') then '2025-06-11'::date
                          when schr.rating = 'N' then '2025-06-11'::date
                          when schr.rating = 'O' then '2025-06-11'::date
                          when schr.rating = 'P' then '2025-06-11'::date
                          else '2025-06-11'::date
                          end
                    end
       else '2025-06-11'::date
...
, case when c.product_flag in ('Arranged Overdraft', 'RP_1', 'RP_2') and sum(case when c.product_flag in ('Arranged Overdraft', 'RP_1', 'RP_2') then coalesce(o.balance,0) else 0 end) over(partition by c.user_id) < 500.22 then 0.461767::float
     ...
     when c.product_flag in ('Unarranged Overdraft') and country in ('AUT') then 0.81860::float
     when c.product_flag in ('Unarranged Overdraft') and country in ('BEL') then 0.84210::float
     when c.product_flag in ('Unarranged Overdraft') and country in ('DEU') then 0.77610::float
     when c.product_flag in ('Unarranged Overdraft') and country in ('ESP') then 0.93170::float
     when c.product_flag in ('Unarranged Overdraft') and country in ('FRA') then 0.87090::float
     when c.product_flag in ('Unarranged Overdraft') and country in ('IRL') then 0.70410::float
     when c.product_flag in ('Unarranged Overdraft') and country in ('ITA') then 0.84320::float
     when c.product_flag in ('Unarranged Overdraft') and country in ('NLD') then 0.77300::float
     when c.product_flag in ('Unarranged Overdraft') and country in ('POL') then 0.92630::float
     when c.product_flag in ('Unarranged Overdraft') and country in ('PRT') then 0.83770::float
     when c.product_flag in ('Unarranged Overdraft') then 0.9048::float
...
, case when c.product_flag in ('RP_1', 'Arranged Overdraft', 'Unarranged Overdraft') then 12
       when r.last_repayment::date < '2025-06-11'::date then 12 -- edge case, late customers payments
       when round(datediff('month', '2025-06-11'::date, r.last_repayment::date)) = 0 then 1 -- edge case, less than 1 month till maturity
       else round(datediff('month', '2025-06-11'::date, r.last_repayment::date)) end as Maturity
...
, case when c.product_flag in ('Arranged Overdraft', 'RP_1') then coalesce(f.first_creation_date::date,'2025-06-11'::date)
       when c.product_flag <> 'Unarranged Overdraft' then coalesce(li.creation_date::date,'2025-06-11'::date)
...
, case when c.is_default = 1 and d.user_id is not null then d.default_date
       when c.is_default = 1 and ins.user_id is not null then ins.internal_insolvency_date
       when c.is_default = 1 and sch.user_id is not null then c.schufa_rating_date
       when c.is_default = 1 and c.dpd > 90 then c.start_in_arrears
       end "DXAUD"
...
, case when d.user_id is not null then d.default_date
       when ins.user_id is not null then ins.internal_insolvency_date
       when sch.user_id is not null then c.schufa_rating_date
       when (df.user_id is not null)
            and (c.reportdate::date between df.default_during_forbearance_date::date and date_add('month', 12, df.default_during_forbearance_date::date)) then df.default_during_forbearance_date::date
       when c.dpd > 90 then c.start_in_arrears
       end "DXNPE"
...
from "n26"."credit_risk_playground"."bp_retail_provisions_stg_v5_pre_updated_ik" c 
left join "n26"."credit_risk_playground"."bp_portfolio_balance_aud_m_ik" o on o.encoded_key = c.instrument_id and c.reportdate::date = o.reporting_date::date
left join "n26"."credit_risk_playground"."bp_overdraft_split_m_ik"  mll on mll.encoded_key = c.instrument_id and c.reportdate::date = mll.reporting_date::date
left join "n26"."credit_risk_playground"."bp_monitoring_pd_supermodel_v2_m_ik" m on m.user_id = c.user_id and m.reporting_date::date = c.reportdate::date
left join "n26"."credit_risk_playground"."bp_application_pd_supermodel_m_ik" pd_app on pd_app.encoded_key = c.instrument_id 
                                                                and  pd_app.reporting_date::date = c.reportdate::date
                                                                and c.product_flag not in ('Unarranged Overdraft', 'RP_2')
left join "n26"."credit_risk_playground"."bp_application_pd_supermodel_m_ad_ik" pd_app_ad on pd_app_ad.encoded_key = c.instrument_id 
                                                                and  pd_app_ad.reporting_date::date = c.reportdate::date
                                                                and c.product_flag not in ('Unarranged Overdraft', 'RP_2')
left join "n26"."credit_risk_playground"."bp_default_dunning_m_ik" d on d.user_id = c.user_id  and d.reporting_date::date = c.reportdate::date   
left join "n26"."credit_risk_playground"."bp_default_internal_insolvency_v2_m_ik" ins on ins.user_id = c.user_id and ins.reporting_date::date = c.reportdate::date
left join "n26"."credit_risk_playground"."bp_default_credit_bureau_insolvency_v3_m_ik" sch on sch.user_id = c.user_id and sch.reporting_date::date = c.reportdate::date
left join "n26"."credit_risk_playground"."bp_repayment_plan_ph1_m_ik" rp on rp.user_id = c.user_id and rp.reporting_date::date = c.reportdate::date
left join "n26"."credit_risk_playground"."bp_credit_bureau_rating_m_ik" schr on schr.user_id = c.user_id and schr.reporting_date::date = c.reportdate::date
left join first_enabled_date f on f.encoded_key = c.instrument_id and f.user_id = c.user_id and f.first_creation_date::date <= '2025-06-11'::date
...
left join credit_risk_playground.porto_pit_app_users pd_od on pd_od.user_id = c.user_id 
                                                              ...
                                                              -- and pd_od.product_flag = c.product_flag
                                                               and pd_od.credit_creation_date::date = coalesce(f.first_creation_date::date,'2025-06-11'::date)
                                                               
left join repayment r on r.parent_account_key = c.instrument_id
left join  "n26"."dbt"."zrh_users" z on z.user_id = c.user_id 
left join "n26"."credit_risk_playground"."bp_loan_contract_information_m_ik" li on li.encoded_key = c.instrument_id and li.reporting_date::date = c.reportdate::date
left join "n26"."public"."mmbr_savings_account" s on s.encoded_key = c.instrument_id and s.account_type = 'CURRENT_ACCOUNT'
left join "n26"."credit_risk_playground"."bp_garnishment_m_ik" g on g.user_id = c.user_id 
                                             and c.reportdate::date = g.reporting_date::date  
left join "n26"."credit_risk_playground"."bp_social_benefits_v2_m_ik" sb on sb.user_id = c.user_id and sb.reporting_date::date = c.reportdate::date
left join "n26"."credit_risk_playground"."bp_p_accounts_ik" pa on pa.user_id = c.user_id and '2025-06-11'::date between pa.rev_timestamp and pa.end_timestamp
left join "n26"."credit_risk_playground"."bp_hrc_m_ik" hrc on c.user_id = hrc.user_id and c.reportdate::date = hrc.reporting_date::date
left join forbearance_last_year fly on fly.instrument_id = c.instrument_id 
left join "n26"."credit_risk_playground"."bp_forbearance_m_v2_ik" fb on fb.instrument_id = c.instrument_id and c.reportdate::date = fb.reportdate::date
...
where c.reportdate::date = '2025-06-11'::date
order by 1
)
...
where etl_updated > (select max(etl_updated) from "n26"."credit_risk_playground"."bp_retail_provisions_stg_v5_v4_updated_ik")
