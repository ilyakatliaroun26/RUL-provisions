...
,  '2025-06-11'::date as ReportDate
,  '2025-06-11'::date as CalcDate -- for retail same as eom, for mortgage 1-month lag
...
, case when product_flag in ('Arranged Overdraft', 'RP_1') then coalesce(f.first_creation_date::date,'2025-06-11'::date)
       when product_flag <> 'Unarranged Overdraft' then coalesce(li.creation_date::date,'2025-06-11'::date)
       else null end credit_creation_date

-- 4. DPD

, start_in_arrears::date as start_in_arrears
, case when start_in_arrears is not null then datediff('day', start_in_arrears::date, '2025-06-11'::date) + 1
       else 0 end dpd
...
, schr.provider
, case when schr.rev_timestamp::date is not null then '2025-06-11'::date
...
, case when pit_calibration_segment in ('RP_phase_2') then 0.10979381443298969
       when credit_creation_date = '2025-06-11'::date 
...
from "n26"."credit_risk_playground"."bp_portfolio_customers_aud_m_ik" c 
left join "n26"."credit_risk_playground"."bp_portfolio_balance_aud_m_ik" o on o.encoded_key = c.encoded_key and c.reporting_date::date = o.reporting_date::date
left join "n26"."credit_risk_playground"."bp_application_pd_supermodel_m_ik" pd_app on pd_app.encoded_key = c.encoded_key 
                                                                and pd_app.reporting_date::date = c.reporting_date::date
left join "n26"."credit_risk_playground"."bp_overdraft_split_m_ik"  mll on mll.encoded_key = c.encoded_key and c.reporting_date::date = mll.reporting_date::date
left join "n26"."credit_risk_playground"."bp_monitoring_pd_supermodel_v2_m_ik" m on m.user_id = c.user_id and m.reporting_date::date = c.reporting_date::date
-- to be fixed - application scores should be from porto
left join "n26"."credit_risk_playground"."bp_default_dunning_m_ik" d on d.user_id = c.user_id  and d.reporting_date::date = c.reporting_date::date   
left join "n26"."credit_risk_playground"."bp_default_internal_insolvency_v2_m_ik" ins on ins.user_id = c.user_id and ins.reporting_date::date = c.reporting_date::date
left join "n26"."credit_risk_playground"."bp_default_credit_bureau_insolvency_v3_m_ik" sch on sch.user_id = c.user_id and sch.reporting_date::date = c.reporting_date::date
left join "n26"."credit_risk_playground"."bp_repayment_plan_ph1_m_ik" rp on rp.user_id = c.user_id and rp.reporting_date::date = c.reporting_date::date
left join "n26"."credit_risk_playground"."bp_credit_bureau_rating_m_ik" schr on schr.user_id = c.user_id and schr.reporting_date::date = c.reporting_date::date
left join dbt.bp_arrears_reg_aud de on de.user_id = c.user_id
                                       and  '2025-06-11'::date between start_in_arrears::date and end_in_arrears::date
left join "n26"."credit_risk_playground"."bp_overdraft_limit_periods_ik" p on c.encoded_key = p.encoded_key 
                                                        and p.amount_cents > 0 
                                                        and date_add('minute', -1, date_add('day', 1, '2025-06-11'::date) ) between p.rev_timestamp and p.end_timestamp
left join first_enabled_date f on f.encoded_key = c.encoded_key and f.user_id = c.user_id and f.first_creation_date::date <= '2025-06-11'::date
left join repayment r on r.parent_account_key = c.encoded_key
left join  "n26"."dbt"."zrh_users" z on z.user_id = c.user_id 
left join "n26"."credit_risk_playground"."bp_loan_contract_information_m_ik" li on li.encoded_key = c.encoded_key and li.reporting_date::date = c.reporting_date::date
left join "n26"."public"."mmbr_savings_account" s on s.encoded_key = c.encoded_key and s.account_type = 'CURRENT_ACCOUNT'
where 
c.reporting_date::date = '2025-06-11'::date
and case when z.closed_at <= '2025-06-11'::date 
...
where etl_updated > (select max(etl_updated) from "n26"."credit_risk_playground"."bp_retail_provisions_stg_v5_pre_updated_ik")
