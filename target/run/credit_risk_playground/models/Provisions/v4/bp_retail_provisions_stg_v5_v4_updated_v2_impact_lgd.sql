  ...
  create  table
    "n26"."credit_risk_playground"."bp_retail_provisions_stg_v5_v4_updated_v2_impact_lgd"
...
where rev_timestamp <= date_add('minute', -1, date_add('day', 1, last_day(date_add('month', -2, last_day(getdate())))) ) and amount_cents > 0 
group by 
user_id 
, encoded_key
)
...
where f.credit_creation_date::date between date_add('month', -12
                                                    , last_day(date_add('month', -2, last_day(getdate())))
                                                    ) and last_day(date_add('month', -2, last_day(getdate())))
      and f.reportdate = last_day(date_add('month', -2, last_day(getdate())))
...
, case when d.user_id is not null or ins.user_id is not null or sch.user_id is not null then last_day(date_add('month', -2, last_day(getdate())))
       when (df.user_id is not null)
            and (c.reportdate::date between df.default_during_forbearance_date::date and date_add('month', 12, df.default_during_forbearance_date::date)) then last_day(date_add('month', -2, last_day(getdate())))
       when c.dpd > 90 then last_day(date_add('month', -2, last_day(getdate())))
       when max(case when c.product_flag in ('RP_2') then 1 else 0 end) over(partition by c.user_id) = 1 then
              case when c.dpd > 90 then last_day(date_add('month', -2, last_day(getdate())))
                  when c.dpd > 60 then  last_day(date_add('month', -2, last_day(getdate())))
                  when c.dpd > 30 then last_day(date_add('month', -2, last_day(getdate())))
                  when c.dpd > 0 then last_day(date_add('month', -2, last_day(getdate())))
                  --when c.rating_class_pit_calibrated < 14 then m.calculated_at::date 
                  else last_day(date_add('month', -2, last_day(getdate())))
               end 
       ...
       when c.pd_pit_calibrated is null then 
            case when c.dpd > 90 then last_day(date_add('month', -2, last_day(getdate())))
                 ...
                 when c.dpd > 0 then last_day(date_add('month', -2, last_day(getdate())))
                     ...
                     case when schr.rating = 'A' then last_day(date_add('month', -2, last_day(getdate())))
                          when schr.rating = 'B' then last_day(date_add('month', -2, last_day(getdate())))
                          when schr.rating = 'C' then last_day(date_add('month', -2, last_day(getdate())))
                          when schr.rating = 'D' then last_day(date_add('month', -2, last_day(getdate())))
                          when schr.rating = 'E' then last_day(date_add('month', -2, last_day(getdate())))
                          when schr.rating = 'F' then last_day(date_add('month', -2, last_day(getdate())))
                          when schr.rating = 'G' then last_day(date_add('month', -2, last_day(getdate())))
                          when schr.rating in ('H','I','K','L', 'M','UNKNOWN') then last_day(date_add('month', -2, last_day(getdate())))
                          when schr.rating = 'N' then last_day(date_add('month', -2, last_day(getdate())))
                          when schr.rating = 'O' then last_day(date_add('month', -2, last_day(getdate())))
                          when schr.rating = 'P' then last_day(date_add('month', -2, last_day(getdate())))
                          else last_day(date_add('month', -2, last_day(getdate())))
                          end
                    end
       else last_day(date_add('month', -2, last_day(getdate())))
...
, CASE
    WHEN c.is_default = 1 AND d.user_id IS NOT NULL
         AND NOT (d.default_date IS NULL AND c.credit_creation_date IS NULL)
         AND GREATEST(
                COALESCE(d.default_date, DATE '0001-01-01'),
                COALESCE(c.credit_creation_date, DATE '0001-01-01')
             ) < date_add('year', -3, c.reportdate)
    THEN 1.0::float

    WHEN c.is_default = 1 AND ins.user_id IS NOT NULL
         AND NOT (ins.internal_insolvency_date IS NULL AND c.credit_creation_date IS NULL)
         AND GREATEST(
                COALESCE(ins.internal_insolvency_date, DATE '0001-01-01'),
                COALESCE(c.credit_creation_date, DATE '0001-01-01')
             ) < date_add('year', -3, c.reportdate)
    THEN 1.0::float

    WHEN c.is_default = 1 AND sch.user_id IS NOT NULL
         AND NOT (c.schufa_rating_date IS NULL AND c.credit_creation_date IS NULL)
         AND GREATEST(
                COALESCE(c.schufa_rating_date, DATE '0001-01-01'),
                COALESCE(c.credit_creation_date, DATE '0001-01-01')
             ) < date_add('year', -3, c.reportdate)
    THEN 1.0::float

    WHEN c.is_default = 1 AND c.dpd > 90
         AND NOT (c.start_in_arrears IS NULL AND c.credit_creation_date IS NULL)
         AND GREATEST(
                COALESCE(date_add('day', 90, c.start_in_arrears), DATE '0001-01-01'),
                COALESCE(c.credit_creation_date, DATE '0001-01-01')
             ) < date_add('year', -3, c.reportdate)
    THEN 1.0::float
  else
...
, case when c.product_flag in ('RP_1', 'Arranged Overdraft', 'Unarranged Overdraft') then 12
       when r.last_repayment::date < last_day(date_add('month', -2, last_day(getdate()))) then 12 -- edge case, late customers payments
       when round(datediff('month', last_day(date_add('month', -2, last_day(getdate()))), r.last_repayment::date)) = 0 then 1 -- edge case, less than 1 month till maturity
       else round(datediff('month', last_day(date_add('month', -2, last_day(getdate()))), r.last_repayment::date)) end as Maturity
...
, case when c.product_flag in ('Arranged Overdraft', 'RP_1') then coalesce(f.first_creation_date::date,last_day(date_add('month', -2, last_day(getdate()))))
       when c.product_flag <> 'Unarranged Overdraft' then coalesce(li.creation_date::date,last_day(date_add('month', -2, last_day(getdate()))))
...
,CASE
    WHEN c.credit_creation_date IS NULL
         AND (
            CASE 
                WHEN c.is_default = 1 AND d.user_id IS NOT NULL THEN d.default_date
                ...
                WHEN c.is_default = 1 AND c.dpd > 90 THEN date_add('day', 90, c.start_in_arrears)
            END
         ) IS NULL
    THEN NULL
    ELSE GREATEST(
...
left join "n26"."credit_risk_playground"."bp_monitoring_pd_supermodel_v3_m_v2" m on m.user_id = c.user_id and m.reporting_date::date = c.reportdate::date
...
left join first_enabled_date f on f.encoded_key = c.instrument_id and f.user_id = c.user_id and f.first_creation_date::date <= last_day(date_add('month', -2, last_day(getdate())))
...
left join credit_risk_playground.porto_pit_app_users pd_od on pd_od.user_id = c.user_id 
                                                              ...
                                                              -- and pd_od.product_flag = c.product_flag
                                                               and pd_od.credit_creation_date::date = coalesce(f.first_creation_date::date,last_day(date_add('month', -2, last_day(getdate()))))
...
left join "n26"."credit_risk_playground"."bp_p_accounts_v2" pa on pa.user_id = c.user_id and last_day(date_add('month', -2, last_day(getdate()))) between pa.rev_timestamp and pa.end_timestamp
...
where c.reportdate::date = last_day(date_add('month', -2, last_day(getdate())))
order by 1
)
