  ...
  create  table
    "n26"."credit_risk_playground"."bp_retail_provisions_stg_v5_v5_updated_v2"
...
, bp_forbearance_m_v2 as (
  select
    f.*
    , coalesce(date_diff('day',dor.start_in_arrears::date, f.credit_creation_date::date),0) as dpd_at_credit_decision
    , cor.rev_timestamp as cb_report_date
    , cor.provider as cb_provider 
    , cor.rating as cb_rating
    , case 
        when coalesce(date_diff('day', dor.start_in_arrears::date, f.credit_creation_date::date),0) > 30 then 'dpd_30+'
        when initialpd >= 0.2790 then 'RC_15+'
        when (cor.provider like 'SCHUFA%' and cor.rating in ('N', 'O', 'P')) then 'cb_financial_difficulty'
        when (cor.provider like 'CRIF%' and cor.rating in ('P')) then 'cb_financial_difficulty'
        when (paor.user_id is not null) then 'p_account'
        when (m.initiator_user_id is not null) then 'unemployment_benefits'
        when (gor.customer_id is not null) then 'garnishment'
        else 'other'
      end as forbearance_reason
  from forbearance_customers f
left join dbt.bp_arrears_reg_aud dor on dor.user_id = f.user_id 
                                      and f.credit_creation_date::timestamp between dor.start_in_arrears::timestamp and dor.end_in_arrears::timestamp
left join californium_credit_score cor on cor.user_id = f.user_id 
                                      and f.credit_creation_date::timestamp between cor.rev_timestamp::timestamp and cor.end_timestamp::timestamp
left join paccount_ts paor on paor.user_id = f.user_id 
                            and f.credit_creation_date::date between paor.rev_timestamp::timestamp and paor.end_timestamp::timestamp
left join garnishment gor on gor.customer_id = f.user_id 
                           and (
                                  (
                                    gor.status = 'CLOSE'
                                      and f.credit_creation_date::timestamp between gor.created::timestamp and gor.updated::timestamp
                                  )
                                  or 
                                  (
                                    gor.status = 'OPEN' and gor.created::timestamp <= f.credit_creation_date::timestamp
                                  )
                                )
left join madrid_categorization m on m.initiator_user_id = f.user_id 
                                      and m.created::date between date_add('day', -91, f.credit_creation_date)::date and f.credit_creation_date::date
...
left join bp_forbearance_m_v2 fb on fb.instrument_id = c.instrument_id and c.reportdate::date = fb.reportdate::date
